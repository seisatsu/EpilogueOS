...
PGM_P const streamname[] PROGMEM = {serialstream, i2cstream,  spistream,
                                    sdstream,     wifistream, stringstream,
                                    gfxstream};
...
void error2(builtin_t fname, PGM_P string) { errorsym2(sym(fname), string); }
...
bool eqsymbols(object *obj, char *buffer) {
  ...
  while (!(arg == NULL && buffer[i] == 0)) {
    if (arg == NULL || buffer[i] == 0 ||
        arg->chars != (buffer[i] << 24 | buffer[i + 1] << 16 |
...
void trace(symbol_t name) {
  if (tracing(name))
    error(TRACE, PSTR("already being traced"), symbol(name));
  int i = 0;
  while (i < TRACEMAX) {
    if (TraceFn[i] == 0) {
      TraceFn[i] = name;
...
object *quote(object *arg) { return cons(bsymbol(QUOTE), cons(arg, NULL)); }
...
symbol_t sym(builtin_t x) { return twist(x + BUILTINS); }
...
uint32_t pack40(char *buffer) {
  int x = 0;
  for (int i = 0; i < 6; i++)
    x = x * 40 + toradix40(buffer[i]);
  return x;
}

bool valid40(char *buffer) {
  if (toradix40(buffer[0]) < 11)
    return false;
  for (int i = 1; i < 6; i++)
    if (toradix40(buffer[i]) < 0)
      return false;
  return true;
}
...
int checkbitvalue(builtin_t name, object *obj) {
  ...
  int n = obj->integer;
  if (n & ~1)
    error(name, PSTR("argument is not a bit value"), obj);
  return n;
}
...
int checkchar(builtin_t name, object *obj) {
  if (!characterp(obj))
    error(name, PSTR("argument is not a character"), obj);
  return obj->chars;
}

object *checkstring(builtin_t name, object *obj) {
  if (!stringp(obj))
    error(name, notastring, obj);
  return obj;
}

int isstream(object *obj) {
  if (!streamp(obj))
    error(NIL, PSTR("not a stream"), obj);
  return obj->integer;
}
...
int checkkeyword(builtin_t name, object *obj) {
  if (!keywordp(obj))
    error(name, PSTR("argument is not a keyword"), obj);
  builtin_t kname = builtin(obj->name);
  uint8_t context = getminmax(kname);
  if (context != 0 && context != name)
    error(name, invalidkey, obj);
...
object *assoc(object *key, object *list) {
  while (list != NULL) {
    if (improperp(list))
      error(ASSOC, notproper, list);
    object *pair = first(list);
    if (!listp(pair))
      error(ASSOC, PSTR("element is not a list"), pair);
...
object *makearray(builtin_t name, object *dims, object *def, bool bitp) {
  ...
  while (dims != NULL) {
    ...
    if (d < 0)
      error2(MAKEARRAY, PSTR("dimension can't be negative"));
    size = size * d;
    dims = cdr(dims);
  }
...
object **getarray(builtin_t name, object *array, object *subs, object *env,
                  int *bit) {
  ...
  *bit = -1;
  ...
  while (dims != NULL && subs != NULL) {
    ...
    if (env)
      s = checkinteger(name, eval(car(subs), env));
    else
      s = checkinteger(name, car(subs));
    if (s < 0 || s >= d)
      error(name, PSTR("subscript out of range"), car(subs));
    size = size * d;
    index = index * d + s;
    dims = cdr(dims);
    subs = cdr(subs);
  ...
  if (dims != NULL)
    error2(name, PSTR("too few subscripts"));
  if (subs != NULL)
    error2(name, PSTR("too many subscripts"));
  if (bitp) {
    size = (size + 31) / 32;
    *bit = index & 0x1F;
    index = index >> 5;
...
void rslice(object *array, int size, int slice, object *dims, object *args) {
  ...
  for (int i = 0; i < d; i++) {
    int index = slice * d + i;
    if (!consp(args))
      error2(NIL, PSTR("initial contents don't match array type"));
    if (cdr(dims) == NULL) {
...
object *readbitarray(gfun_t gfun) {
  ...
  while (!issp(ch) && ch != ')' && ch != '(') {
    if (ch != '0' && ch != '1')
      error2(NIL, PSTR("illegal character in bit array"));
  ...
  while (head != NULL) {
    ...
    int bit = index & 0x1F;
    *loc =
        number((((*loc)->integer) & ~(1 << bit)) | (car(head)->integer) << bit);
    index++;
    head = cdr(head);
  }
  return array;
...
void pslice(object *array, int size, int slice, object *dims, pfun_t pfun,
  ...
  for (int i = 0; i < d; i++) {
    ...
    if (cdr(dims) == NULL) {
      if (bitp)
        pint(((*arrayref(array, index >> 5, size))->integer) >> (index & 0x1f) &
                 1,
             pfun);
      else
...
void pstr(char c) { buildstring(c, &GlobalStringTail); }
...
char *cstring(object *form, char *buffer, int buflen) {
  ...
  while (form != NULL) {
    ...
    for (int i = (sizeof(int) - 1) * 8; i >= 0; i = i - 8) {
      ...
      if (ch) {
        if (index >= buflen - 1)
          error2(NIL, PSTR("no room for string"));
        buffer[index++] = ch;
      }
    }
    form = car(form);
...
bool boundp(object *var, object *env) {
  ...
  if (value(varname, env) != NULL)
    return true;
  if (value(varname, GlobalEnv) != NULL)
    return true;
  return false;
}
...
object *findvalue(object *var, object *env) {
  ...
  if (pair == NULL)
    pair = value(varname, GlobalEnv);
  if (pair == NULL)
    error(NIL, PSTR("unknown variable"), var);
  return pair;
}

// Handling closures

object *closure(int tc, symbol_t name, object *function, object *args,
                object **env) {
  ...
  *bit = -1;
  ...
  if (symbolp(function)) {
    ...
    if (sname == sym(CAR) || sname == sym(FIRST)) {
      ...
      if (!listp(value))
        error(name, canttakecar, value);
      return &car(value);
    }
    if (sname == sym(CDR) || sname == sym(REST)) {
      object *value = eval(second(args), env);
      if (!listp(value))
        error(name, canttakecdr, value);
      return &cdr(value);
    }
    if (sname == sym(NTH)) {
      ...
      if (atom(list))
        error(name, PSTR("second argument to nth is not a list"), list);
      while (index > 0) {
        list = cdr(list);
        if (list == NULL)
          error2(name, PSTR("index to nth is out of range"));
        index--;
      }
      return &car(list);
    }
    if (sname == sym(AREF)) {
      ...
      if (!arrayp(array))
        error(AREF, PSTR("first argument is not an array"), array);
...
bool I2Cstart(uint8_t address, uint8_t read) {
  ...
  if (read == 0) {
    ...
    Wire.requestFrom(address, I2CCount);
  return ok;
}
...
void I2Cstop(uint8_t read) {
  if (read == 0)
    Wire.endTransmission(); // Check for error?
}
...
gfun_t gstreamfun(object *args) {
  ...
  if (streamtype == I2CSTREAM)
    ...
    error2(NIL, PSTR("unknown stream type"));
  return gfun;
}
...
#elif defined(ESP32)
  if (!(pin == 0 || pin == 2 || pin == 4 || (pin >= 12 && pin <= 15) ||
...
void tone(int pin, int note) { (void)pin, (void)note; }
...
void playnote(int pin, int note, int octave) {
  ...
  if (prescaler < 0 || prescaler > 8)
    error(NOTE, PSTR("octave out of range"), number(prescaler));
  tone(pin, pgm_read_word(&scale[note % 12]) >> prescaler);
}

void nonote(int pin) { noTone(pin); }

// Sleep

void initsleep() {}

void sleep(int secs) { vTaskDelay(pdMS_TO_TICKS(1000 * secs)); }
...
bool quoted(object *obj) {
  return (consp(obj) && car(obj) != NULL && car(obj)->name == sym(QUOTE) &&
...
const char ppspecial[ppspecials] PROGMEM = {
    DOTIMES, DOLIST,     IF,        SETQ,      TEE,     LET,
    LETSTAR, LAMBDA,     WHEN,      UNLESS,    WITHI2C, WITHSERIAL,
    WITHSPI, WITHSDCARD, FORMILLIS, WITHCLIENT};
...
void supersub(object *form, int lm, int super, pfun_t pfun) {
  ...
  if (symbolp(arg)) {
    ...
    if (sname == sym(DEFUN))
      ...
      for (int i = 0; i < ppspecials; i++) {
        if (sname == sym((builtin_t)pgm_read_byte(&ppspecial[i]))) {
          special = 1;
          break;
        }
      }
...
object *sp_incf(object *args, object *env) {
  ...
  if (bit != -1) {
    ...
    if (inc == NULL)
      ...
      increment = checkbitvalue(INCF, inc);
    int newvalue = (((*loc)->integer) >> bit & 1) + increment;

    if (newvalue & ~1)
      error2(INCF, PSTR("result is not a bit value"));
  ...
  if (floatp(x) || floatp(inc)) {
    ...
    if (inc == NULL)
      ...
      increment = checkintfloat(INCF, inc);
...
object *sp_decf(object *args, object *env) {
  ...
  if (bit != -1) {
    ...
    if (dec == NULL)
      ...
      decrement = checkbitvalue(DECF, dec);
  ...
  if (floatp(x) || floatp(dec)) {
    ...
    if (dec == NULL)
      ...
      decrement = checkintfloat(DECF, dec);
...
object *sp_setf(object *args, object *env) {
  ...
  while (args != NULL) {
    ...
    if (bit == -1)
      ...
      *loc = number((checkinteger(SETF, *loc) & ~(1 << bit)) |
                    checkbitvalue(SETF, arg) << bit);
    args = cddr(args);
  }
  return arg;
}
...
object *sp_dolist(object *args, object *env) {
  if (args == NULL || listlength(DOLIST, first(args)) < 2)
    error2(DOLIST, noargument);
...
object *sp_dotimes(object *args, object *env) {
  if (args == NULL || listlength(DOTIMES, first(args)) < 2)
    error2(DOTIMES, noargument);
  object *params = first(args);
  object *var = first(params);
...
object *sp_formillis(object *args, object *env) {
  if (args == NULL)
    error2(FORMILLIS, noargument);
  object *param = first(args);
  unsigned long start = millis();
  unsigned long now, total = 0;
  if (param != NULL)
    total = checkinteger(FORMILLIS, eval(first(param), env));
...
object *sp_withoutputtostring(object *args, object *env) {
  if (args == NULL)
    error2(WITHOUTPUTTOSTRING, noargument);
  object *params = first(args);
  if (params == NULL)
    error2(WITHOUTPUTTOSTRING, nostream);
  object *var = first(params);
...
object *sp_withserial(object *args, object *env) {
  object *params = first(args);
  if (params == NULL)
    error2(WITHSERIAL, nostream);
  object *var = first(params);
  int address = checkinteger(WITHSERIAL, eval(second(params), env));
  params = cddr(params);
  int baud = 96;
  if (params != NULL)
    baud = checkinteger(WITHSERIAL, eval(first(params), env));
...
object *sp_withi2c(object *args, object *env) {
  object *params = first(args);
  if (params == NULL)
    error2(WITHI2C, nostream);
  object *var = first(params);
  int address = checkinteger(WITHI2C, eval(second(params), env));
  params = cddr(params);
  if (address == 0 && params != NULL)
    params = cdr(params); // Ignore port
  int read = 0;           // Write
  I2CCount = 0;
  if (params != NULL) {
  ...
  I2Cinit(1); // Pullups
  object *pair =
      cons(var, (I2Cstart(address, read)) ? stream(I2CSTREAM, address) : nil);
  push(pair, env);
  object *forms = cdr(args);
  object *result = eval(tf_progn(forms, env), env);
  I2Cstop(read);
...
object *sp_withspi(object *args, object *env) {
  ...
  if (params != NULL) {
    ...
    params = cdr(params);
    if (params != NULL) {
      bitorder = (checkinteger(WITHSPI, eval(car(params), env)) == 0)
                     ? LSBFIRST
                     : MSBFIRST;
      params = cdr(params);
      if (params != NULL) {
  ...
  SPI.beginTransaction(
      SPISettings(((unsigned long)clock * 1000), bitorder, mode));
...
object *sp_withsdcard(object *args, object *env) {
  ...
  if (params != NULL && first(params) != NULL)
    mode = checkinteger(WITHSDCARD, first(params));
  ...
  if (mode >= 1) {
    ...
    if (!SDpfile)
      error2(WITHSDCARD,
             PSTR("problem writing to SD card or invalid filename"));
  } else {
    char buffer[BUFFERSIZE];
    SDgfile = SD.open(MakeFilename(filename, buffer), oflag);
    if (!SDgfile)
      error2(WITHSDCARD,
             PSTR("problem reading from SD card or invalid filename"));
  }
  ...
  if (params == NULL) {
    ...
    if (stringp(address))
      success = client.connect(cstring(address, buffer, BUFFERSIZE),
                               checkinteger(WITHCLIENT, port));
    else if (integerp(address))
      success =
          client.connect(address->integer, checkinteger(WITHCLIENT, port));
    else
      error2(WITHCLIENT, PSTR("invalid address"));
    if (!success)
      return nil;
    n = 1;
  }
...
object *tf_if(object *args, object *env) {
  if (args == NULL || cdr(args) == NULL)
    error2(IF, PSTR("missing argument(s)"));
...
object *tf_cond(object *args, object *env) {
  while (args != NULL) {
    ...
    if (!consp(clause))
      error(COND, PSTR("illegal clause"), clause);
...
object *tf_case(object *args, object *env) {
  ...
  while (args != NULL) {
    object *clause = first(args);
    if (!consp(clause))
      error(CASE, PSTR("illegal clause"), clause);
...
object *fn_length(object *args, object *env) {
  ...
  object *arg = first(args);
  if (listp(arg))
    return number(listlength(LENGTH, arg));
  if (stringp(arg))
...
object *fn_arraydimensions(object *args, object *env) {
  ...
  if (!arrayp(array))
    error(ARRAYDIMENSIONS, PSTR("argument is not an array"), array);
  object *dimensions = cddr(array);
...
object *fn_makearray(object *args, object *env) {
  ...
  while (args != NULL && cdr(args) != NULL) {
    ...
    if (isbuiltin(first(args), INITIALELEMENT))
      def = second(args);
    else if (isbuiltin(first(args), ELEMENTTYPE) &&
             isbuiltin(second(args), BIT))
      bitp = true;
    else
      error(MAKEARRAY, PSTR("argument not recognised"), var);
    args = cddr(args);
  }
  if (bitp) {
    if (def == nil)
      ...
      def = number(-checkbitvalue(MAKEARRAY, def)); // 1 becomes all ones
  }
...
object *fn_nth(object *args, object *env) {
  ...
  if (n < 0)
    error(NTH, indexnegative, first(args));
  object *list = second(args);
  while (list != NULL) {
    if (improperp(list))
      error(NTH, notproper, list);
    if (n == 0)
      return car(list);
    list = cdr(list);
    n--;
  ...
  while (args != NULL) {
    ...
    while (consp(list)) {
      ...
      if (cdr(args) != NULL && improperp(list))
        error(APPEND, notproper, first(args));
    }
    args = cdr(args);
  }
  return head;
...
object *divide_floats(object *args, float fresult) {
  while (args != NULL) {
    ...
    if (f == 0.0)
      error2(DIVIDE, PSTR("division by zero"));
    fresult = fresult / f;
    args = cdr(args);
  }
  return makefloat(fresult);
...
object *fn_mod(object *args, object *env) {
  ...
  if (integerp(arg1) && integerp(arg2)) {
    ...
    if (divisor == 0)
      error2(MOD, PSTR("division by zero"));
  ...
  while (args != NULL) {
    ...
    if (integerp(result) && integerp(arg)) {
      if ((arg->integer) > (result->integer))
        result = arg;
    } else if ((checkintfloat(MAXFN, arg) > checkintfloat(MAXFN, result)))
      result = arg;
    args = cdr(args);
  }
  return result;
...
object *fn_minfn(object *args, object *env) {
  ...
  while (args != NULL) {
    ...
    if (integerp(result) && integerp(arg)) {
      if ((arg->integer) < (result->integer))
        result = arg;
    } else if ((checkintfloat(MINFN, arg) < checkintfloat(MINFN, result)))
      result = arg;
    args = cdr(args);
  }
  return result;
    ...
    while (nargs != NULL) {
      ...
      if (integerp(arg1) && integerp(arg2)) {
        if ((arg1->integer) == (arg2->integer))
          return nil;
      } else if ((checkintfloat(NOTEQ, arg1) == checkintfloat(NOTEQ, arg2)))
        return nil;
      nargs = cdr(nargs);
    }
    args = cdr(args);
...
object *compare(builtin_t name, object *args, bool lt, bool gt, bool eq) {
  ...
  while (args != NULL) {
    ...
    if (integerp(arg1) && integerp(arg2)) {
      ...
      if (!lt && (checkintfloat(name, arg1) < checkintfloat(name, arg2)))
        return nil;
      if (!eq && (checkintfloat(name, arg1) == checkintfloat(name, arg2)))
        return nil;
      if (!gt && (checkintfloat(name, arg1) > checkintfloat(name, arg2)))
        return nil;
    }
    arg1 = arg2;
    args = cdr(args);
...
object *fn_atan(object *args, object *env) {
  ...
  args = cdr(args);
  if (args != NULL)
    div = checkintfloat(ATAN, first(args));
...
object *fn_log(object *args, object *env) {
  ...
  if (args == NULL)
    ...
    return makefloat(fresult / log(checkintfloat(LOG, first(args))));
}
...
object *fn_expt(object *args, object *env) {
  ...
  if (integerp(arg1) && integerp(arg2) && ((arg2->integer) > 0) &&
      (abs(value) < 21.4875))
  ...
  if (float1 < 0) {
    if (integerp(arg2))
      ...
      error2(EXPT, PSTR("invalid result"));
  }
...
object *fn_ceiling(object *args, object *env) {
  ...
  args = cdr(args);
  if (args != NULL)
    return number(ceil(checkintfloat(CEILING, arg) /
                       checkintfloat(CEILING, first(args))));
  else
    return number(ceil(checkintfloat(CEILING, arg)));
}
...
object *fn_floor(object *args, object *env) {
  ...
  if (args != NULL)
    return number(
        floor(checkintfloat(FLOOR, arg) / checkintfloat(FLOOR, first(args))));
  else
    return number(floor(checkintfloat(FLOOR, arg)));
}
...
object *fn_truncate(object *args, object *env) {
  ...
  args = cdr(args);
  if (args != NULL)
    return number((int)(checkintfloat(TRUNCATE, arg) /
                        checkintfloat(TRUNCATE, first(args))));
  else
    return number((int)(checkintfloat(TRUNCATE, arg)));
}
...
object *fn_round(object *args, object *env) {
  ...
  if (args != NULL)
    return number(
        myround(checkintfloat(ROUND, arg) / checkintfloat(ROUND, first(args))));
  else
    return number(myround(checkintfloat(ROUND, arg)));
}
...
object *fn_sort(object *args, object *env) {
  ...
  while (cdr(ptr) != NULL) {
    ...
    while (go != ptr) {
      ...
      if (apply(SORT, predicate, compare, env))
        break;
      go = cdr(go);
    }
    if (go != ptr) {
...
object *fn_subseq(object *args, object *env) {
  ...
  if (start < 0)
    error(SUBSEQ, indexnegative, second(args));
  int end;
  args = cddr(args);
  if (args != NULL)
    end = checkinteger(SUBSEQ, car(args));
  else
    end = stringlength(arg);
  object *result = newstring();
  object *tail = result;
  for (int i = start; i < end; i++) {
    char ch = nthchar(arg, i);
    if (ch == 0)
      error2(SUBSEQ, indexrange);
    buildstring(ch, &tail);
  }
  return result;
}
...
object *fn_eval(object *args, object *env) { return eval(first(args), env); }
...
object *fn_makunbound(object *args, object *env) {
  ...
  object *var = first(args);
  if (!symbolp(var))
    error(MAKUNBOUND, notasymbol, var);
...
object *fn_restarti2c(object *args, object *env) {
  ...
  if (stream >> 8 != I2CSTREAM)
    error2(RESTARTI2C, PSTR("not an i2c stream"));
...
object *fn_pinmode(object *args, object *env) {
  ...
  object *arg = first(args);
  if (keywordp(arg))
    pin = checkkeyword(NIL, arg);
  else
    pin = checkinteger(PINMODE, first(args));
  int pm = INPUT;
  arg = second(args);
  if (keywordp(arg))
    pm = checkkeyword(PINMODE, arg);
  else if (integerp(arg)) {
...
object *fn_digitalwrite(object *args, object *env) {
  ...
  if (keywordp(arg))
    ...
    pin = checkinteger(DIGITALWRITE, arg);
  arg = second(args);
  int mode;
  if (keywordp(arg))
    mode = checkkeyword(DIGITALWRITE, arg);
  else if (integerp(arg))
...
object *fn_analogread(object *args, object *env) {
  ...
  object *arg = first(args);
  if (keywordp(arg))
    pin = checkkeyword(ANALOGREAD, arg);
  else {
...
object *fn_analogwrite(object *args, object *env) {
  ...
  if (keywordp(arg))
    ...
    pin = checkinteger(ANALOGWRITE, arg);
...
object *fn_note(object *args, object *env) {
  ...
  if (args != NULL) {
    ...
    int note = 0;
    if (cddr(args) != NULL)
      note = checkinteger(NOTE, second(args));
    int octave = 0;
    if (cddr(args) != NULL)
      octave = checkinteger(NOTE, third(args));
...
object *fn_require(object *args, object *env) {
  ...
  while (line != NULL) {
    ...
    if ((fname == sym(DEFUN) || fname == sym(DEFVAR)) &&
        symbolp(second(line)) && second(line)->name == arg->name) {
      eval(line, env);
      return tee;
    }
    line = read(glibrary);
...
object *fn_available(object *args, object *env) {
  ...
  if (isstream(first(args)) >> 8 != WIFISTREAM)
    error2(AVAILABLE, PSTR("invalid stream"));
...
object *fn_wifisoftap(object *args, object *env) {
  ...
  if (args == NULL)
    return WiFi.softAPdisconnect(true) ? tee : nil;
  object *first = first(args);
  args = cdr(args);
  if (args == NULL)
    WiFi.softAP(cstring(first, ssid, 33));
  else {
    ...
    if (args != NULL) {
      ...
      args = cdr(args);
      if (args != NULL)
        hidden = (first(args) != nil);
    }
    WiFi.softAP(cstring(first, ssid, 33), cstring(second, pass, 65), channel,
                hidden);
  }
...
object *fn_connected(object *args, object *env) {
  (void)env;
  if (isstream(first(args)) >> 8 != WIFISTREAM)
    error2(CONNECTED, PSTR("invalid stream"));
...
object *fn_wificonnect(object *args, object *env) {
  ...
  if (result == WL_CONNECTED)
    ...
    error2(WIFICONNECT, PSTR("network not found"));
  else if (result == WL_CONNECT_FAILED)
    error2(WIFICONNECT, PSTR("connection failed"));
  else
    error2(WIFICONNECT, PSTR("unable to connect"));
  return nil;
}
...
object *fn_drawpixel(object *args, object *env) {
  ...
  if (cddr(args) != NULL)
    colour = checkinteger(DRAWPIXEL, third(args));
  tft.drawPixel(checkinteger(DRAWPIXEL, first(args)),
                checkinteger(DRAWPIXEL, second(args)), colour);
  tft.display();
#endif
  return nil;
}
...
object *fn_drawline(object *args, object *env) {
  ...
  if (args != NULL)
    colour = checkinteger(DRAWLINE, car(args));
...
object *fn_drawrect(object *args, object *env) {
  ...
  if (args != NULL)
    colour = checkinteger(DRAWRECT, car(args));
...
object *fn_fillrect(object *args, object *env) {
  ...
  if (args != NULL)
    colour = checkinteger(FILLRECT, car(args));
...
object *fn_drawcircle(object *args, object *env) {
  ...
  if (args != NULL)
    colour = checkinteger(DRAWCIRCLE, car(args));
...
object *fn_fillcircle(object *args, object *env) {
  ...
  if (args != NULL)
    colour = checkinteger(FILLCIRCLE, car(args));
...
object *fn_drawroundrect(object *args, object *env) {
  ...
  if (args != NULL)
    colour = checkinteger(DRAWROUNDRECT, car(args));
  tft.drawRoundRect(params[0], params[1], params[2], params[3], params[4],
                    colour);
  tft.display();
#endif
  return nil;
...
object *fn_fillroundrect(object *args, object *env) {
  ...
  if (args != NULL)
    colour = checkinteger(FILLROUNDRECT, car(args));
  tft.fillRoundRect(params[0], params[1], params[2], params[3], params[4],
                    colour);
  tft.display();
#endif
  return nil;
...
object *fn_drawtriangle(object *args, object *env) {
  ...
  if (args != NULL)
    colour = checkinteger(DRAWTRIANGLE, car(args));
  tft.drawTriangle(params[0], params[1], params[2], params[3], params[4],
                   params[5], colour);
  tft.display();
#endif
  return nil;
...
object *fn_filltriangle(object *args, object *env) {
  ...
  if (args != NULL)
    colour = checkinteger(FILLTRIANGLE, car(args));
  tft.fillTriangle(params[0], params[1], params[2], params[3], params[4],
                   params[5], colour);
  tft.display();
#endif
  return nil;
...
object *fn_drawchar(object *args, object *env) {
  ...
  if (more != NULL) {
    ...
    if (more != NULL) {
      ...
      more = cdr(more);
      if (more != NULL)
        size = checkinteger(DRAWCHAR, car(more));
    }
  }
  tft.drawChar(checkinteger(DRAWCHAR, first(args)),
               checkinteger(DRAWCHAR, second(args)),
               checkchar(DRAWCHAR, third(args)), colour, bg, size);
  tft.display();
#endif
  return nil;
}
...
#if defined(gfxsupport)
  (void)env;
  tft.setCursor(checkinteger(SETCURSOR, first(args)),
                checkinteger(SETCURSOR, second(args)));
#endif
  return nil;
}
...
object *fn_settextcolor(object *args, object *env) {
  ...
  (void)env;
  if (cdr(args) != NULL)
    tft.setTextColor(checkinteger(SETTEXTCOLOR, first(args)),
                     checkinteger(SETTEXTCOLOR, second(args)));
  else
    tft.setTextColor(checkinteger(SETTEXTCOLOR, first(args)));
#endif
  return nil;
...
object *fn_fillscreen(object *args, object *env) {
  ...
  if (args != NULL)
    colour = checkinteger(FILLSCREEN, first(args));
...
void checkminmax(builtin_t name, int nargs) {
  ...
  if (nargs < (minmax >> 4))
    error2(name, toofewargs);
  if ((minmax & 0x0f) != 0x0f && nargs > (minmax & 0x0f))
    error2(name, toomanyargs);
}

void testescape() {
  if (Serial.read() == '~')
    error2(NIL, PSTR("escape!"));
}
...
EVAL:
  ...
  if (function == NULL)
    error(NIL, PSTR("illegal function"), nil);
  if (!listp(args))
    error(NIL, PSTR("can't evaluate a dotted pair"), args);
  ...
  if (symbolp(function)) {
    ...
    if ((name == LET) || (name == LETSTAR)) {
      int TCstart = TC;
      if (args == NULL)
        error2(name, noargument);
      object *assigns = first(args);
      if (!listp(assigns))
        error(name, notalist, assigns);
      object *forms = cdr(args);
      object *newenv = env;
      push(newenv, GCStack);
      while (assigns != NULL) {
        ...
        if (!consp(assign))
          ...
          push(cons(first(assign), eval(second(assign), env)), newenv);
        car(GCStack) = newenv;
        if (name == LETSTAR)
          env = newenv;
        assigns = cdr(assigns);
    ...
    if (((name > 0) && (name < SPECIAL_FORMS)) ||
        ((name > KEYWORDS) && (name < USERFUNCTIONS)))
  ...
  if (symbolp(function)) {
    ...
    if (!builtinp(function->name))
      error(NIL, PSTR("not valid here"), fname);
  ...
  error(NIL, PSTR("illegal function"), fname);
  return nil;
}
...
const char ControlCodes[] PROGMEM =
    ...
    "Page\0Return\0SO\0SI\0DLE\0DC1\0DC2\0DC3\0DC4\0NAK\0SYN\0ETB\0CAN\0EM\0SUB"
    "\0Escape\0FS\0GS\0RS\0US\0Space\0";
...
void plispstring(object *form, pfun_t pfun) { plispstr(form->name, pfun); }
...
void printsymbol(object *form, pfun_t pfun) { psymbol(form->name, pfun); }
...
void psymbol(symbol_t name, pfun_t pfun) {
  if ((name & 0x03) == 0)
    ...
    if (value < PACKEDS)
      error2(NIL, PSTR("invalid symbol"));
...
inline void pln(pfun_t pfun) { pfun('\n'); }
...
void printobject(object *form, pfun_t pfun) {
  if (form == NULL)
    ...
    error2(NIL, PSTR("error in print"));
}
...
object *nextitem(gfun_t gfun) {
  ...
  // Parse reader macros
    ...
    if (ch == '\\') {      // Character
      ...
      error2(NIL, PSTR("illegal character after #"));
    ch = gfun();
  }
  ...
  if (x != ENDFUNCTIONS)
    return bsymbol(x);
  else if ((index <= 6) && valid40(buffer))
    return intern(twist(pack40(buffer)));
  buffer[index + 1] = '\0';
  buffer[index + 2] = '\0';
  buffer[index + 3] = '\0'; // For internlong
  ...
  while (item != (object *)KET) {
    if (item == (object *)BRA) {
      ...
      if (readrest(gfun) != NULL)
        error2(NIL, PSTR("malformed list"));
      return head;
    } else {
...
object *read(gfun_t gfun) {
  object *item = nextitem(gfun);
  if (item == (object *)KET)
    error2(NIL, PSTR("incomplete list"));
  if (item == (object *)BRA)
...
void setup() {
  ...
  initgfx();
  init_vfs();
  pfstring(PSTR("uLisp 4.0 "), pserial);
  pln(pserial);
  spawnshell();
}
...
void repl(object *env) {
  for (;;) {
    ...
    if (line == (object *)KET)
      error2(NIL, PSTR("unmatched right bracket"));
    push(line, GCStack);
    pfl(pserial);
    line = eval(line, env);
    pfl(pserial);
...
void spawnshell() {
  ...
  xTaskCreate(taskrepl, "_SHELL_", 4096, NULL, 1, &xHandle);
...
ps_tbl_entry_t *getps() {
  ...
  for (std::vector<ps_tbl_entry_t>::iterator it = ProcessTable.begin();
       it != ProcessTable.end(); ++it) {
